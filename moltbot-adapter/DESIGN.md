# Moltbot Adapter 详细设计文档

## 1. 概述

### 1.1 项目定位

moltbot-adapter是野火IM与Moltbot Gateway之间的桥接适配器，负责：
- 双向消息格式转换
- 连接管理和自动重连
- 用户/群组白名单过滤
- 群聊防刷屏策略
...
└───────────────────────────┬─────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│              MoltbotAdapter (moltbot-adapter)                    │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                    MoltbotBridge                         │  │
│  │  ┌──────────────────┐  ┌──────────────────────────────┐ │  │
│  │  │ WildfireClient   │  │ MoltbotWebSocketClient       │ │  │
│  │  │ (SDK内置)        │  │ - 连接管理                    │ │  │
│  │  │ - 消息接收       │  │ - 认证握手                    │ │  │
│  │  │ - 自动重连       │  │ - 心跳保活                    │ │  │
│  │  │ - 消息发送       │  │ - 事件处理                    │ │  │
...
// Moltbot消息处理
void onConnected()                // Moltbot连接成功
void onDisconnected(int, String)  // Moltbot连接断开
void onResponse(MoltbotInMessage) // 接收AI响应

// 状态查询
boolean isRunning()               // 桥接器运行状态
boolean isWildfireConnected()     // 野火连接状态
boolean isMoltbotConnected()      // Moltbot连接状态
```
...
2. WhitelistFilter.shouldProcess()
   ↓
3. MessageConverter.convertToMoltbot()
   ↓
4. GroupFilter.shouldRespond()
   ↓
5. MoltbotWebSocketClient.sendMessage()
```

**消息流转（Moltbot → 野火）**：
```
1. onResponse(MoltbotInMessage)
   ↓
2. MessageConverter.convertFromMoltbot()
   ↓
...
- 重连次数：最多10次
- 重连间隔：`moltbot.reconnect.interval`（默认5秒）
- 超时时间：每次重连最多等待5秒认证完成

### 2.2 MoltbotWebSocketClient（WebSocket客户端）
...
handleAgentEvent(JsonObject):
  ...
  3. 构建MoltbotInMessage(type="streaming")
...
handleChatEvent(JsonObject):
  ...
  2. 查找messageContexts[runId]
  3. if state="final":
       - 构建MoltbotInMessage(type="response")
...
**Moltbot → 野火转换**：
```java
convertFromMoltbot(MoltbotInMessage):
  1. 判断会话类型
...
**配置参数**：
```properties
moltbot.whitelist.enabled=true                  # 是否启用白名单
moltbot.whitelist.allowed-users=user123,user456 # 允许的用户ID列表
moltbot.whitelist.allowed-groups=group789       # 允许的群组ID列表
```
...
0. WhitelistFilter.shouldProcess()   [第0步：全局白名单]
   ↓
1. MessageConverter.convertToMoltbot()  [第1步：格式转换]
   ↓
...
**配置参数**：
```properties
moltbot.group.enabled=true                    # 总开关
moltbot.group.respond-on-mention=true         # 被@时回复
moltbot.group.respond-on-question=true        # 问号结尾回复
moltbot.group.help-keywords=帮,请,分析,总结,怎么,如何
moltbot.group.allowed-ids=groupId1,groupId2   # 白名单
```
...
moltbot.whitelist.allowed-users=user123,user456
moltbot.whitelist.allowed-groups=group789,group101
```

#### GroupStrategyConfig
```java
@ConfigurationProperties(prefix = "moltbot.group")
- enabled: true
...
#### SessionConfig
```java
@ConfigurationProperties(prefix = "moltbot.session")
...
```json
{
  ...
  "params": {
    ...
    "userAgent": "moltbot-adapter/1.0.0"
  }
}
```
...
**间隔**：`moltbot.heartbeat.interval`（默认30秒）
...
### 5.2 MoltbotHealthController

**接口**：`GET /moltbot/status`
...
```json
{
  "message": "Moltbot Adapter is running",
  "wildfire": "Connected" | "Disconnected",
  "moltbot": "Connected" | "Disconnected"
}
```
...
**MoltbotReconnectThread**：
- 触发：连接断开时
- 生命周期：重连完成或超时后结束
- 守护线程：否

**MoltbotHeartbeatThread**：
- 启动：认证成功后
- 间隔：30秒
- 守护线程：是
...
```properties
# 使用环境变量或配置外部化
moltbot.gateway.token=${MOLTBOT_TOKEN:}
```

### 8.2 输入验证

- 消息内容长度限制（由Moltbot Gateway控制）
- JSON解析异常捕获
- NPE防护
...
| moltbot.authenticated | Moltbot认证状态 | true |
| bridge.running | 桥接器运行状态 | true |

### 10.2 告警建议

- **连接断开**：wildfire或moltbot连接断开超过1分钟
- **认证失败**：Moltbot认证连续失败3次
- **消息错误率**：消息发送失败率 > 5%
...
logging.level.cn.wildfirechat.moltbot=INFO
