package cn.wildfirechat.moltbot.session;

import cn.wildfirechat.moltbot.config.GroupStrategyConfig;
...
public class GroupFilter {
     ...
     */
    public boolean shouldRespond(MoltbotOutMessage message, String robotId) {
     ...
     * 检测消息是否提及了机器人
     */
    private boolean isMentioned(MoltbotOutMessage message, String robotId) {
        if (robotId == null || message.getMessage().getMentions() == null) {
            return false;
        }

        for (MoltbotOutMessage.Message.Mention mention : message.getMessage().getMentions()) {
